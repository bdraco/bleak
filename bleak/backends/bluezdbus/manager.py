            ...
            try:
                ...
                if interface == defs.DEVICE_INTERFACE:
                    ...
                    if device_condition_callbacks:
                        logger.warning(
                            "device %s changed properties %s (%s) - new: %s",
                            device_path,
                            changed,
                            device_condition_callbacks,
                            self_interface,
                        )
                        for prop_name, callback in device_condition_callbacks:
                            if prop_name in changed:

                                callback(self_interface.get(prop_name))
                    ...
                    if "Connected" in changed:
                        ...
                        logger.warning(
                            "device %s changed connection state to %s (%s)",
                            device_path,
                            new_connected,
                            watchers
                        )
                    ...
                    if "Value" in changed:
                        ...
                        watchers = self._device_watchers.get(device_path)
                        logger.warning(
                            "device %s (%s) changed connection state to %s (%s)",
                            device_path,
                            message_path,
                            new_value,
                            watchers
